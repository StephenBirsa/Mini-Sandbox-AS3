package
{
	...
	public class OnScreenTrace extends Sprite
	{
		...
		public function OnScreenTrace(screenWidth:Number, screenHeight:Number, colour:int = 0x000000, boldOutline:Boolean = false, boldColour:int = 0xFFFFFF) 
		{
			super();
			_boldColour = boldColour;
		...
		public function set boldOutline(b:Boolean):void {
			bold = b;
			if (bold == true) {
				glow = new GlowFilter(_boldColour, 1, 2, 2, 10, BitmapFilterQuality.MEDIUM);
				_trace.filters = [glow];
			}
			if (bold == false) {
				if (glow != null) {
